    ...
    if 'flame' in conditions.config:
        ...
        if red_data.red_op.sens_method == 'adjoint':
            ...
            def get_species_reaction_sensitivities(f, tsp, grid_point):
                ...
                try:
                    i_spec = f.inlet.n_components + \
                        f.flame.component_index(
                            tsp) + f.domains[1].n_components*grid_point
                except:
                    i_spec = f.fuel_inlet.n_components + \
                        f.flame.component_index(
                            tsp) + f.domains[1].n_components*grid_point
...
#                    S_react_x_zi = f.solve_adjoint(perturb, f.gas.n_reactions, dgdx,g,1e-5) * (kf/spec_0)
                S_react_x_zi = f.solve_adjoint(
                    perturb, f.gas.n_reactions, dgdx, g, 5e-2) / spec_0
            ...
            for z in range(n_points):
                if z % int(max(n_points/red_data.red_op.n_points, 1)) == 0    \
                    ...
                    if verbose >= 2:
                        bar.update(z)
                    sensi_T_z_adj = get_species_reaction_sensitivities(
                        f, 'T', z)
                    ...
                    for spA in range(n_spc):
                        if mech_data.spec.activ_m[spA]:
                            if 'SARGEP' in red_data.reduction_operator:
                                S_react_x_adj[spA] = get_species_reaction_sensitivities(
                                    f, gas_ref.species_name(spA), z)
                            else:
                                S_react_x_adj[spA] = get_species_reaction_sensitivities(
                                    f, tsp_name[spA], z)
                            r_red = 0
                                        ...
                                        if mech_data.react.activ_m[r]:
                                            ...
                                            for spB in range(n_sp_ref):
                                                if (mech_data.spec.activ_m[spB] and nu[spB, r] != 0):
                                                    ...
                                                    S_AB_tsp_z_r[z_i][r][spA,
                                                                         spB] = S_react_x[z_i][spA][r]
                    z_i += 1
            ...
            time_2 = timer.time()
            if verbose >= 4:
                print_("\n      time for adjoint method SA computation: " +
                       str(round(time_2-time_1))+'s', mp)
#            except:
        ...
        if red_data.red_op.sens_method == 'brute_force':
            ...
            for r in range(n_r_ref):
                if mech_data.react.activ_m[r]:
                    ...
                    r_red += 1
                    # set the multiplier of kf for r reaction
                    f.gas.set_multiplier(1+dk, r_red)
                    ...
                    if 'diff' in conditions.config:
                        phi = 'diff'
                    else:
                        phi = '%.2f' % conditions.composition.phi
...
#                                        *((T[z]-red_results.T[z])/(kf_pert-kf_ref))
                                sensi_T_z[z_i][r] = (
                                    T[z]-red_results.T[z])/(red_results.T[z]*dk)

                                # Species sensitivity to reactions
                                for spA in range(n_sp_ref):
                                    if mech_data.spec.activ_m[spA]:
                                        if 'SARGEP' in red_data.reduction_operator:
                                            ired = f.gas.species_index(
                                                gas_ref.species_name(spA))
                                            # tol_ss_flame[1] : atol for steady-state problem
                                            if conc_pert[ired] > conditions.simul_param.tol_ss[1]:
                                                #            		                                S_react_x[z_i][spA][r]=kf_ref/conc_red[spA]\
                                                #            		                                  *((np.max([conc_pert[ired],0])-conc_red[spA])\
                                                #            		                                     /(kf_pert-kf_ref))
                                                S_react_x[z_i][spA][r] = (
                                                    np.max([conc_pert[ired], 0])-conc_red[spA])/(conc_red[spA]*dk)
                                            ...
                                            if spA in tsp_idx:
                                                ...
                                                if conc_pert[ired] > conditions.simul_param.tol_ss[1]:
                                                    ...
                                                    for spB in range(n_sp_ref):
                                                        if (mech_data.spec.activ_m[spB] and nu[spB, r] != 0):
                                                            # collecting sensitivities of reactions involving both species
                                                            S_AB_tsp_z_r[z_i][r][tsp,
                                                                                 spB] = S_react_x[z_i][tsp][r]
                            z_i += 1
                bar.update(r)

            time_2 = timer.time()
            if verbose >= 4:
                print_("\n      time for brute-force SA computation: " +
                       str(round(time_2-time_1))+'s', mp)
            os.chdir(cwd)
...
#       Sensitivity analysis
        ...
        if 'SARGEP' in red_data.reduction_operator:
            n_spc = n_sp_ref
            if verbose >= 2:
                bar = cdef.ProgressBar(
                    mech_data.spec.activ_m.count(True), title)
        else:
            n_spc = n_tsp
            if verbose >= 2:
        ...
        for spA in range(n_spc):
            if mech_data.spec.activ_m[spA]:
                sensi_tsp_done = False
                sp_c += 1
                # initialisation of the matrix of interactions between A and B at z
                S_AB_tsp_z = np.zeros((n_points_SA, n_sp_ref))
                            ...
                            if z % int(n_points/red_data.red_op.n_points) == 0\
                                ...
                                for r in range(n_r_ref):
                                    ...
                                    if not sensi_T_done:  # to do only at the first loop
                                        ...
                                        norm_sensi_T_z[z_i][r] = sensi_T_z[z_i][r] / \
                                            max_sensi_T_z
                                    ...
                                    if not sensi_tsp_done and max_sensi_tsp_z > 1e-10:  # to do only at the first spB loop
                                        ...
                                        norm_S_react_x[z_i][spA][r] = S_react_x[z_i][spA][r] / \
                                            max_sensi_tsp_z
                                        # keep the max sens coeffs of all z points
                ...
                for z in range(n_points):
                    if z % int(n_points/red_data.red_op.n_points) == 0\
                        ...
                        if S_AB_tsp_z_max > 1e-10:
                            for spB in range(n_sp_ref):
                                S_AB_tsp_z[z_i][spB] = S_AB_tsp_z[z_i][spB] / \
                                    S_AB_tsp_z_max
        ...
        if 'SARGEP' in red_data.reduction_operator:
            ...
            #S_AB_t = np.zeros((n_points_SA, n_tsp, n_sp_ref))

        # fuel1 index
        ind_fuel = gas_red.species_index(
            conditions.composition.fuel.split('/')[0].split('(')[0])
        fuel_conc = [gas_red.concentrations[ind_fuel]]

        bar = cdef.ProgressBar(
            n_points-int(n_points/red_data.red_op.n_points-1))
        t_i = -1
    ...
    #            S_AB_t    = np.zeros((n_sp,n_sp))
                ...
                for spA in range(n_spc):
                    if 'SARGEP' not in red_data.reduction_operator or mech_data.spec.activ_m[spA]:
                            ...
                            if mech_data.react.activ_m[r]:
                                ...
                                for spB in range(n_sp_ref):
                                    if mech_data.spec.activ_m[spB] and nu[spB, r] != 0:
                                        ...
                                        if conditions.config == "reactor_UV":
                                            S_AB_tsp_t[t_i][spA, spB] += abs(
                                                sensi_r_t[sp_red+3, r_red])
                                        elif conditions.config == "reactor_HP":
                                            S_AB_tsp_t[t_i][spA, spB] += abs(
                                                sensi_r_t[sp_red+2, r_red])
                        ...
                        S_react_x_2w[t_i][spA] = copy.deepcopy(
                            S_react_x[t_i][spA])
                        max_S_react_x = max(abs(S_react_x[t_i][spA]))
                        if max_S_react_x > 0:
                            for r in range(n_r_ref):
                                S_react_x[t_i][spA][r] = S_react_x[t_i][spA][r] / \
                                    max_S_react_x
...
#                print_('Warning: error during in sensitivities calculation at t = '+'%.3f'%(pts_scatter[t]*1000)+' ms',mp)
        ...
        if conditions.error_param.ig_check:
            if red_results.ign_time_hr != False:
                ign0 = red_results.ign_time_hr
            else:
                ign0 = red_results.ign_time_sp
            ...
            for r in range(n_r_ref):
                if mech_data.react.activ_m[r]:
                    ...
                    r_red += 1
                    # set the multiplier of kf for r reaction
                    gas_red.set_multiplier(1+dk, r_red)
                    ...
                    if red_results.ign_time_hr != False:
                        ignp = pert_results.ign_time_hr
                    else:
                        ignp = pert_results.ign_time_sp

                    # Ignition delay sensitivity
                    # nondimensional sensitivity coefficient
                    sensi_igt[r] = (ignp-ign0)/(ign0*dk)
        ...
        # flag to avoid S_react matrix unfilled when time iterations and fuel reactivity are decorelated
        S_react_filled = False
                    ...
                    if t != 0 and t % int(n_points/red_data.red_op.n_points) == 0:
                        ...
                        try:  # ... if error during in sensitivities calculation
                            if 0.01*abs(fuel_conc[0]-fuel_conc[-1])     \
                                ...
                                # ~ (C_fuel_ti+1%)<C_fuel<(C_fuel_tf-1%)
...
#            X_red = conditions.composition.molarFraction(gas_red)
        ...
        stirredReactor = ct.IdealGasReactor(
            gas_red, energy='off', volume=reactorVolume)
        ...
        columnNames = [stirredReactor.component_name(
            item) for item in range(stirredReactor.n_vars)]
...
#            bar.update(np.log10(maxSimulationTime))
                ...
                for spA in range(n_spc):
                    if 'SARGEP' not in red_data.reduction_operator or mech_data.spec.activ_m[spA]:
                            ...
                            if mech_data.react.activ_m[r]:
                                ...
                                for spB in range(n_sp_ref):
                                    if mech_data.spec.activ_m[spB] and nu[spB, r] != 0:
                                        ...
                                        # (sum of the sensitivities of reactions involving both species)
                                        S_AB_tsp_t[t_i][spA,
                                                        spB] += abs(sensi_r_t[sp_red+3, r_red])
                        ...
                        if max_S_react_x > 0:
                            ...
                            print_('all sens computed = 0 at T: ' +
                                   str(T_list[t]) + ' for '+tsp_name[tsp], mp)
                        # S_AB_t sensitivity normalisation on t
                        max_S_AB_tsp_t = max(abs(S_AB_tsp_t[t_i][spA]))
                        if max_S_AB_tsp_t > 0:
                            for spB in range(n_sp_ref):
    ...
    if conditions.simul_param.verbose >= 4:
        print_("\n      time for SA computation+analysis: " +
               str(round(time_end-time_start))+'s', mp)
    ...
    if red_data.red_op.write_results:
        ...
        write_sensitivities(red_data, sensi_scatter, S_react_x_2w,
                            sensi_T_z_2w, sensi_igt, S_AB_tsp_z_r, red_results)
...
#    target_species = red_data.targetSpeciesIdx
    ...
    if conditions.composition.X2:
        ...
        init_spec = init_spec.split(',')
        for spec in init_spec:
            ind_spec = gas_ref.species_index(
    ...
    if '_r' in red_method:
        for tsp in range(len(tsp_idx)):
            ...
            sensi_sorted.sort()
            while 0 in sensi_sorted:
                sensi_sorted.remove(0)
        ...
        if conditions.error_param.Sl_check:
            try:
                ...
                while 0 in sensi_Sl_sorted:
                    sensi_Sl_sorted.remove(0)
        ...
        if conditions.error_param.T_check:
            try:
                ...
                sensi_T_sorted.sort()
                while 0 in sensi_T_sorted:
                    sensi_T_sorted.remove(0)
        ...
        if conditions.error_param.ig_check:
            try:
                ...
                while 0 in sensi_ig_sorted:
                    sensi_ig_sorted.remove(0)
...
def write_sensitivities(red_data, sensi_scatter, S_react_x, T_react_z, igt_react, S_AB_tsp_z_r, red_results):
    ...
    for s in sensi_scatter:
        ...
        if "flame" in config or "PFR" in config:
            l0_conditions = 'z = ' + \
                '%1.3e' % red_results.pts_scatter[s] + ' (m)\n'
        elif "JSR" in config:
            l0_conditions = 'T = ' + \
                '%1.3e' % red_results.pts_scatter[s] + ' (K)\n'
        elif "reactor" in config:
            l0_conditions = 't = ' + \
                '%1.3e' % red_results.pts_scatter[s] + ' (s)\n'
        ...
        for r_i in range(len(S_react_x[0][0])):
            txt = str(r_i+1) + ';' + \
                conditions.composition.gas.reaction_equations([r_i])[0]
            if "free_flame" in config:
                if type(red_data.red_op.sensi_Sl) is not bool:
                    txt += ';' + \
                        '%0.5e' % T_react_z[s_i-1][r_i] + ';' + \
                        '%0.5e' % red_data.red_op.sensi_Sl[r_i]
                else:
...
def plot_sentitivities(sp2plot, red_results, mech_data, sensi_scatter, S_react_x, T_react_z,
    ...
    for sp in sp2plot:
        ...
        if len(sens_pos) != 0:
            ...
            if reaction_2plot == "all":
                reaction_2plot = len(sens_pos)
            sens_vect = []
            reactions = []
            n_neg = 0
            n_pos = 0
            for r in range(reaction_2plot):
                ...
                if sens_pos[n_pos][1] > abs(sens_neg[n_neg][1]):
                    sens_vect.append(sens_pos[n_pos][1])
                    n_pos += 1
                    if reaction_label == "number":
                        reactions.append(
                            str(mech_data.react.number[sens_pos[n_pos][0]]))
                    elif reaction_label == "equation":
                        reactions.append(
                            str(mech_data.react.equation[sens_pos[n_pos][0]]))
                    ...
                    sens_vect.append(sens_neg[n_neg][1])
                    n_neg += 1
                    if reaction_label == "number":
                        reactions.append(
                            str(mech_data.react.number[sens_neg[n_neg][0]]))
                    elif reaction_label == "equation":
                        reactions.append(
                            str(mech_data.react.equation[sens_neg[n_neg][0]]))
                    elif reaction_label == "reaction+equation":
        ...
        for sp in sp2plot:
            ...
            if len(sens_pos) != 0 and sp != 'Sl':
                ...
                if 'reactor' in red_results.conditions.config:
                    xlabel += 't = ' + \
                        '%0.2f' % (red_results.pts_scatter[s]*1000)+' ms'
                elif 'flame' in red_results.conditions.config:
                    xlabel += 'z = ' + \
                        '%0.1f' % (red_results.pts_scatter[s]*1000)+' mm'
