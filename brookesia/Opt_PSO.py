...
def PSO(conditions_list,mech_data,ref_results_list,red_data_list):
    ...
    # Reference ind
    mech_data = ot.get_uncertainty(mech_data, optim_param, conditions_list, 'PSO')

    ref_ind = ot.Individual(conditions_list,mech_data,\
                         ref_results_list,red_data_list,'PSO',False)
    ref_ind.fitness = ref_ind.fitness_eval(conditions_list,optim_param,ref_results_list,'PSO')
    ...
    best_ind.mech.react.uncert = copy.deepcopy(pop.individual[0].mech.react.uncert)
    ...
    # save and display convergence information
    ...
    for n_it in range(1, optim_param.n_gen+1):
        ...
        pop = ot.fitness_eval_newchilds(pop,optim_param,conditions_list,ref_results_list,n_it,'PSO')
        pop.particle_experience()
        pop,best_ind,new_best_ind = ot.compare_best_ind(pop,best_ind,optim_param,'PSO',verbose)        
        if new_best_ind and optim_param.exp_data:                              # optimization of the time iteration for reactor models
            best_ind.fitness = best_ind.fitness_eval(conditions_list,optim_param,ref_results_list, 'PSO')
    ...
    Opt_results_list, errors_list, fitness = \
        best_ind.export_data(conditions_list,optim_param,ref_results_list,'PSO')
...
class Population:
    def __init__(self,conditions_list,mech_data,red_data_list,ref_results_list,\
        ...
        # option to import modified mechanisms in the first population
        ...
        for ind in range(size_pop):
            ...
            self.individual.append(ot.Individual(conditions_list,mech_data,\
                                   ref_results_list,red_data_list,'PSO',rand_kin))
            # print(ind)
    ...
    def evolution(self, optim_param,best_ind,n_it):
        ...
        worst_fitness = self.individual[0].fitness
        best_fitness  = self.individual[0].fitness
        best_ind = copy.deepcopy(self.individual[0])
        fitness_list = []
        for p_i in range(len(self.individual)):
            fitness_list.append(self.individual[p_i].fitness)
            if self.individual[p_i].fitness < worst_fitness:
                worst_fitness = self.individual[p_i].fitness
            elif self.individual[p_i].fitness > best_fitness:
        ...
        for p_i in range(len(self.individual)):
            ...
            if inertia_score:
                score = (self.individual[p_i].fitness - worst_fitness) / (best_fitness - worst_fitness)
            ...
            for r in range(size_react):
                ...
                if self.individual[0].mech.react.modif[r]:
                    ...
                    or self.individual[0].mech.react.type[r] =='reaction':
                        valid = False; try_r = 1
                        prev_val   = copy.deepcopy(self.individual[p_i].mech.react.kin[r])
                        prev_speed = copy.deepcopy(self.individual[p_i].particle_speed[r])
                        while (valid == False and try_r<5):
                            new_val = []; speed = []
                            for k in range(3):
                                part_current_val =  self.individual[p_i].mech.react.kin[r][k]
                                part_best_val    =  self.individual[p_i].best_mech.react.kin[r][k]
                                pop_best_val     =  best_ind.mech.react.kin[r][k]
                                part_speed       =  self.individual[p_i].particle_speed[r][k]

                                speed.append(inertia * part_speed)                                    # inertia
                                speed[-1] += cognitive_accel * r1* (part_best_val - part_current_val) # cognitive acceleration
                                speed[-1] += social_accel    * r2* (pop_best_val  - part_current_val) # global acceleration
                                speed[-1] /= try_r

                                new_val.append(part_current_val + speed[-1])
                                ...
                                # if abs(new_val) < abs(ref*(1-uncert_r[k])):
                                #     if new_val>0:  new_val = ref*(1-uncert_r[k])
                                #     else:          new_val = ref*(1+uncert_r[k])
                                # elif abs(new_val) > abs(ref*(1+uncert_r[k])):
                                #     if new_val>0:  new_val = ref*(1+uncert_r[k])
                                #     else:          new_val = ref*(1-uncert_r[k])

                            # Validation of the new reaction rate
                            self.individual[p_i].mech.react.kin[r] = new_val
                            # try:
                            valid = ot.check_k(self.individual[p_i].mech.react, r)
                            # except:
                            #     print(str(r))
                        ...
                        if valid != False:
                            self.individual[p_i].mech.react.kin[r] = new_val
                            self.individual[p_i].particle_speed[r] = speed
                        else:
                            self.individual[p_i].mech.react.kin[r] = prev_val
                            self.individual[p_i].particle_speed[r] = prev_speed
                    ...
                    or self.individual[0].mech.react.type[r] =='pdep_arrhenius':
                        ...
                        while (valid == False and try_r < 5):
                            ...
                            for j in range(len(self.individual[0].mech.react.kin[r])):
                                new_val.append([]); speed.append([])
                                for k in range(3):
                                    part_current_val =  self.individual[p_i].mech.react.kin[r][j][k]
                                    part_best_val    =  self.individual[p_i].best_mech.react.kin[r][j][k]
                                    pop_best_val     =  best_ind.mech.react.kin[r][j][k]
                                    part_speed       =  self.individual[p_i].particle_speed[r][j][k]

                                    speed[-1].append(inertia  * part_speed)                             # inertia
                                    speed[-1][-1] += cognitive_accel * r1* (part_best_val - part_current_val) # cognitive acceleration
                                    speed[-1][-1] += social_accel    * r2* (pop_best_val  - part_current_val) # global acceleration
                                    speed[-1][-1] /=  try_r

                                    new_val[-1].append(part_current_val + speed[-1][-1])

                                    # # check whether the new particle value exceeds the boundary, if so => stay stuck at the boundary
                                    # ref = self.individual[0].mech.react.ref_kin[r][j][k]
                                    # if abs(new_val) < abs(ref*(1-uncert_r[k])):
                                    #     if new_val>0:  new_val = ref*(1-uncert_r[k])#+(ref*(1-uncert_r[k])-new_val)
                                    #     else:          new_val = ref*(1+uncert_r[k])#-(ref*(1-uncert_r[k])-new_val)
                                    #     speed = -speed
                                    # elif abs(new_val) > abs(ref*(1+uncert_r[k])):
                                    #     if new_val>0:  new_val = ref*(1+uncert_r[k])#-(ref*(1-uncert_r[k])-new_val)
                                    #     else:          new_val = ref*(1-uncert_r[k])#+(ref*(1-uncert_r[k])-new_val)
                        ...
                        if valid != False:
                            ...
                            self.individual[p_i].particle_speed[r] = prev_speed
                                ...
                                # self.individual[p_i].mech.react.kin[r][j][k] = new_val
                                # self.individual[p_i].particle_speed[r][j][k] = speed


    def particle_experience(self):
        for p_i in range(len(self.individual)):
            if self.individual[p_i].fitness > self.individual[p_i].best_fitness:
                self.individual[p_i].best_fitness = self.individual[p_i].fitness
                self.individual[p_i].best_mech = copy.deepcopy(self.individual[p_i].mech)
    ...
    def compare_best_ind(self, best_ind, optim_param, verbose=0):
        ...
        if self.individual[best_idx].fitness > best_ind.fitness:
            best_ind = copy.deepcopy(self.individual[best_idx])
            os.chdir(mp+'/PSO')
...
#                     self.particle_speed[r][k] = ref*uncert_r[k]*random.uniform(-1, 1)*(10/Max_it)
#
...
#                         self.particle_speed[-1][j][k] = ref*uncert_r[k]*random.uniform(-1, 1)*(10/Max_it)
#             else:
...
#
#         self.mech.react.uncert = []
#         for r in range(len(self.mech.react.formula)):
#             react_found=False
#             for r_inc in range(len(uncertainty)):
#                 if int(num_react[r_inc])==self.mech.react.number[r]\
#                 and uncertainty[r_inc]!='':
#                     self.mech.react.uncert.append([float(u) for u in uncertainty[r_inc].split(',')])
#                     react_found = True
#                     break
#             if not react_found:
#                 self.mech.react.uncert.append(optim_param.Arrh_max_variation)
#         os.chdir("PSO")
#
#
...
#
#         try:
#             fitness = self.individual[ind].fitness_eval(conditions_list,optim_param,ref_results_list,ind)
#         except:
#             fitness = 0
#
...
#             fit_eval_inp.append([optim_param,conditions_list,ref_results_list,bar,ch,True,'PSO'])
#
...
# #            fit_list.append([self.individual[ch].fitness_eval(conditions_list,optim_param,ref_results_list,ch),ch])
#
#
#         # Parallelisation 1 ---------------------------------------------------
...
#             sim_results = pool.map(ot.fitness_eval_par, fit_eval_inp timeout=simul_time_limit)
#             try:
