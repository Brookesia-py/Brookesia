...
def dic(red_data,mech_data,results):
    ...
    if gas_red.n_species > 25 :
        ...
        bar.update(0)
    drg_i=[]
    n_points = len(results.pts_scatter)
    ...
    for i in range(len(results.pts_scatter)):
        if i%int(max(n_points/red_data.red_op.n_points,1))==0 and i!=0:
            drg_i.append(i)
            ib.append(i)
            dic_par_arg_i.append([dic_par_arg,results.kf[i],results.kr[i],results.conc[i],i])
...
def ric(red_data, mech_data, red_results):
    ...
    gas_ref  = red_data.gas_ref
    verbose=red_data.verbose
    n_points = len(red_results.pts_scatter)
...
def reactionWithdrawal(mech_data,red_data,red_method,eps_r,conditions,active_species=False):
    """
