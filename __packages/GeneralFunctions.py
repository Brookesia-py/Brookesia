...
import __packages.Computation as comp
import __packages.SA as sa
import __packages.DRG as drg
import __packages.GeneticOptimisation as go
import gc
import copy
import datetime
...
def reduction(conditions_list,ref_results_list,red_data_list,mech_data):
    ...
    for op in range(len(red_data_list)):
        ...
        if red_method == 'DRG_sp':
            ...
            folderCreation(conditions_list[0].mech,'','','_','DRG_sp',verbose)
        if red_method == 'DRGEP_sp':
            print_('            Species DRGEP reduction \n',mp)
            folderCreation(conditions_list[0].mech,'','','_','DRGEP_sp',verbose)
        elif red_method == 'DRG_r':
            print_('            Reaction DRG reduction \n',mp)
            folderCreation(conditions_list[0].mech,'','','_','DRG_r',verbose)
        elif red_method == 'SAR_sp':
            print_('            Species SA reduction \n',mp)
            folderCreation(conditions_list[0].mech,'','','_','SAR_sp',verbose)
        elif red_method == 'SARGEP_sp':
            print_('            Species SARGEP reduction \n',mp)
            folderCreation(conditions_list[0].mech,'','','_','SARGEP_sp',verbose)
        elif red_method == 'SAR_r':
            print_('            Reaction SA reduction \n',mp)
            folderCreation(conditions_list[0].mech,'','','_','SAR_r',verbose)
    ...
    #                    sp = tspc[idx]
                    ...
                    or errors.above_tol\
                    or max(eps)<=min(eps_init)/100\
                    or max(sp_try)>100 or T_try>100 or ig_try>100 or Sl_try>100 or K_try>100:
                        stop_reduction=True
            ...
            for i in range(len(conditions_list)):
                ...
                red_results_list[i].gas = ct.Solution(filename)
        ...
        if red_data_list[op][0].optim:
            ...
            clock_opt = cdef.Clock('GA') ; clock_opt.start()
            mech_data.react.kin,opt_results_list,opt_errors_list,fitness\
            = go.geneticAlgorithm(conditions_list,mech_data,\
        ...
        for l in range(len(red_results_list)):
            ...
            if red_data_list[op][0].optim:
                opt_results_list[l].write_case_data('Optimisation',op,\
...
def input_results_treatment(conditions_list,ref_results_list):
    ...
    ''' return:
     ...
     -> to define the point scattering for future simulations (reduction/optimisation)
...
# =============================================================================
    ...
    while 'Operators' not in txt[-1] and txt[0] != '':
        ...
        while 'Case' not in txt[-1] and 'Operators' not in txt[-1] and txt[0] != '':
            ...
            if txt[0] == 'xmax':              xmax            = float(txt[1]);  caution_opt_fflame=False
...
# =============================================================================
#     # Reduction / optimisation operators
# =============================================================================
