...
class Red_operator :
    def __init__(self,target_species,optimisation=False,n_points=20,          \
...
#        gas_red           = red_data.gas_loop
        ...
        if error_calculation =="points":
            ...
            for k in range(n_tspc):
                ...
                if sum(data1)>0:      # absence of data if experimental optimisation
                    sumDiff = 0
                    if error_type == "all":
                        sumDiff = []
                        ...
                        for j in range(len(pts_scatter)):
                            sumDiff += abs(data1[j] - data2[j])#/np.amax(data1)
                        #sumDiff = sumDiff/len(pts_scatter)
                        #sumDiff = sumDiff/np.amax(data1)
                        sumDiff/=np.sum(data1)
                    elif error_type == "max":
